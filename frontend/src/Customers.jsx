...
import CustomerCRMReports from './CustomerCRMReports';
...
export default function Customers({ setToast }) {
  ...
  // Tab Control
  const [activeTab, setActiveTab] = useState('directory'); // 'directory', 'payment', 'reports'
  ...
  return (
    <div>
      ...
      <div className="category-tabs" style={{ marginBottom: '24px' }}>
        <button 
          className={`category-tab ${activeTab === 'directory' ? 'active' : ''}`}
          onClick={() => {
        ...
        <button 
          className={`category-tab ${activeTab === 'payment' ? 'active' : ''}`}
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '24px', flexWrap: 'wrap', gap: '16px' }}>
            <div className="search-box-container" style={{ width: '320px' }}>
              ...
              <input
                ...
                placeholder="Search CRM by name, phone, code..."
                value={searchQuery}
                ...
                <table className="table-glass">
                  ...
                  <tbody>
                    {filteredCustomers.length === 0 ? (
                      <tr>
                        <td colSpan={canManage ? 9 : 8} style={{ textAlign: 'center', color: 'var(--text-muted)' }}>
      ...
      {/* TAB 2: Record Payment Checkout */}
      {activeTab === 'payment' && (
        <CustomerPayments
          selectedCustomerFromDir={preselectedCustomer}
          onBackToDirectory={() => {
            setActiveTab('directory');
            setPreselectedCustomer(null);
          }}
      ...
      {/* TAB 3: CRM Reports Panel */}
      {activeTab === 'reports' && (
        <CustomerCRMReports setToast={setToast} />
      )}
    </div>
  );
}
